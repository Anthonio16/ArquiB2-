...
\addbibresource{/home/jhanavi/ArquiB2/bibliography/bibliography.bib}
\usetheme{default}
\author{Almeida Anthonny, Sagñay Micaela y Molina Jhanavi}
\date{2026-06-12}
\title{Taller — Máquina IAS / Von Neumann}
\subtitle{ICCD332 Arquitectura de Computadores}
\hypersetup{
 pdfauthor={Almeida Anthonny, Sagñay Micaela y Molina Jhanavi},
...
\label{sec:orgde5b469}

La máquina de Von Neumann realiza operaciones en un ciclo repetitivo de
\alert{captación}, \alert{decodificación} y \alert{ejecución}. Para esto, el computador
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|cccc|}
    ...
    \multicolumn{4}{|l|}{\textit{Transferencia de datos}} \\ \hline
    00001010 & 0x0A       &  LOAD MQ          & $[AC]\leftarrow[MQ]$\\qq
    ...
    00000100 & 0x04       &  LOAD -|M(X)|     & $[AC]\leftarrow-|[X]|$\\ \hline
    \multicolumn{4}{|l|}{\textit{Salto incondicional}} \\ \hline
    00001101 & 0x0D       &  JUMP M(X,0:19)   & $[PC]\leftarrow$ M(X) izq.\\
    00001110 & 0x0E       &  JUMP M(X,20:39)  & $[PC]\leftarrow$ M(X) der.\\ \hline
    \multicolumn{4}{|l|}{\textit{Salto condicional}} \\ \hline
    00001111 & 0x0F       &  JUMP+ M(X,0:19)  & $\text{si }[AC]\geq 0:\ [PC]\leftarrow$ M(X) izq.\\
    00010000 & 0x10       &  JUMP+ M(X,20:39) & $\text{si }[AC]\geq 0:\ [PC]\leftarrow$ M(X) der.\\ \hline
    \multicolumn{4}{|l|}{\textit{Aritmética}} \\ \hline
    ...
    00001100 & 0x0C       &  DIV M(X)         & $[MQ]\leftarrow[AC]/[X];\ [AC]\leftarrow$ resto\\
    00010100 & 0x14       &  LSH              & $[AC]\leftarrow 2\times[AC]$\\
    00010101 & 0x15       &  RSH              & $[AC]\leftarrow[AC]/2$\\ \hline
    \multicolumn{4}{|l|}{\textit{Modificación de direcciones}} \\ \hline
    00010010 & 0x12       &  STOR M(X,8:19)   & $\text{M(X)}_{8:19}\leftarrow[AC]_{28:39}$\\
    00010011 & 0x13       &  STOR M(X,28:39)  & $\text{M(X)}_{28:39}\leftarrow[AC]_{28:39}$\\
    \hline
  \end{tabular}
\end{table}
...
siguientes preguntas considerando la Tabla \ref{tab:org40a7892} como el
mapa de memoria de la máquina y obtenga el programa en lenguaje ensamblador y en
RTL.

\begin{table}[htbp]
\caption{\label{tab:org40a7892}Mapa de Memoria}
\centering
\begin{tabular}{rrrrr}
\hline
...
\item Le corresponde a LSH en la instrucción 14, esta operación desplaza todos los bits una posición
hacia la izquiera, al realizar este desplazamiento, el valor almacenado en el acumulador se
multiplica por 2, como antes de ejecutar LSH el acumulador tenía el valor 3, en binario corresponde
0011, al desplazar los bits una posición a la izquierda se obtiene 0110, que equivale a 6 en decimal,
por esta razón después de ejecutar LSH el acumulador pasa de 3 a 6.
\end{itemize}
\end{enumerate}
\end{frame}

\begin{frame}[label={sec:orgf16b01e}]{Código RTL}
Escriba en notación de transferencia de registros el programa que
ejecuta el computador IAS.

\begin{itemize}
\item \([AC] \leftarrow [M(940)]\)
\item \([AC] \leftarrow [AC] - [M(941)]\)
\item \([M(940)] \leftarrow [AC]\)
\item \([AC] \leftarrow 2 \times [AC]\)
\item \([M(941)] \leftarrow [AC]\)
\item \([AC] \leftarrow [M(940)]\)
\item \([M(940)] \leftarrow [AC]\)
\item \(\text{HALT}\)
\end{itemize}
...
\begin{frame}[label={sec:org5424507},fragile]{Código Assembler}
 Escriba el código Assembler del programa que ejecuta el computador IAS.

\begin{verbatim}
LOAD M(940)     ; 0x300 izq -> [AC] = 0000000005
SUB M(941)      ; 0x300 der -> [AC] = 0000000005 - 0000000002 = 0000000003
STOR M(940)     ; 0x301 izq -> [940] = 0000000003
LSH             ; 0x301 der -> [AC] = 0000000003 * 2 = 0000000006
STOR M(941)     ; 0x302 izq -> [941] = 0000000006
LOAD M(940)     ; 0x302 der -> [AC] = 0000000003
STOR M(940)     ; 0x303 izq -> [940] = 0000000003
HALT            ; 0x303 der -> fin del programa
\end{verbatim}
\end{frame}

\section{Equipo de Trabajo}
\label{sec:org6b5f31d}
...
\hline
Anthonny Almeida & anthonny.almeida@epn.edu.ec & Tabla de instrucciones IAS, tramo dir 0x300\\[0pt]
Jhanavi Molina & jhanavi.molina@epn.edu.ec & Resultado AC, registros sobrescritos, carry\\[0pt]
Micaela Sagñay & micaela.sagnay@epn.edu.ec & Codigo RTL y Assembler, tramo dir. 0x303,\\[0pt]
 &  & exportacion PDF y entrega.\\[0pt]
\hline
\end{tabular}
\end{center}

\section{Verificación de Entregables [100\%]}
\label{sec:org744101d}
